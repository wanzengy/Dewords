...
class MySegFormer(nn.Layer):
    """
    The MySegFormer implementation based on PaddlePaddle.

    The original article refers to
    Xie, Enze, et al. "MySegFormer: Simple and Efficient Design for Semantic Segmentation with Transformers." arXiv preprint arXiv:2105.15203 (2021).
...
@manager.MODELS.add_component
def MySegFormer_B5(**kwargs):
    return MySegFormer(
        backbone=manager.BACKBONES['MixVisionTransformer_B5'](),
        embedding_dim=768,
        **kwargs)

def segformer_b2(num_classes=1000, pretrained=False):
    if pretrained:
        url = 'https://bj.bcebos.com/paddleseg/dygraph/mix_vision_transformer_b2.tar.gz'
    else:
        url = None
    return MySegFormer_B2(num_classes=num_classes, pretrained=url)


def segformer_b1(num_classes=1000, pretrained=False):
    if pretrained:
        url = 'https://bj.bcebos.com/paddleseg/dygraph/mix_vision_transformer_b1.tar.gz'
    else:
        url = None
    return MySegFormer_B1(num_classes=num_classes, pretrained=url)

if __name__ == '__main__':
    net = segformer_b2(num_classes=1, pretrained=False)
